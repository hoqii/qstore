...
app.factory('productService', ['$http', function($http) {
	return {
		find : function(ct, q, callback) {
			$http.get(ns + 'api/p.json?max=100&c=' + ct + '&q=' + q).success(callback);
		}
	};
}]);
...
app.controller('qstore-catalog', ['$scope', '$http', '$sce', '$filter', 'productService', 'categoryService',
 	...
 	categoryService.find(function(data) {
		...
		productService.find(ct, q, function(data) {
	 		$scope.products = data.products.content;
	 	});
	});
